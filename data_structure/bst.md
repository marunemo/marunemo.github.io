...
그 예시로 좌측 서브 트리의 높이가 3이고 우측 서브 트리의 높이가 1인 경우를 생각해보자.
이 경우에는 right rotate를 해야하는데, 
만약 왼쪽 자식 노드의 오른쪽 자식 노드의 높이가 2 이상이라면,
rotate를 한 뒤에도 해당 노드의 높이가 변하지 않기 때문에 균형은 여전히 맞지 않게 된다.
이번 예시에서는 rotate를 실행하면 좌측 서브 트리의 높이가 1이 되지만,
...
struct BST {
    ...
    Node<T>* _insert(Node<T>* node, T value) {
        ...
        if(node->value > value) {
            ...
            if(node != root && (node->color == 'R' && node->left->color == 'R'))
                doubleRed = true;
        }
        else {
            node->right = _insert(node->right, value);
            node->right->parent = node;
            if(node != root && (node->color == 'R' && node->right->color == 'R'))
                doubleRed = true;
        }
        ...
        if(node->parent->right == node) {
            if(!node->parent->left || node->parent->left->color == 'B') {
                if(node->left && node->left->color == 'R')
                    RightLeft = true;
                else if(node->right && node->right->color == 'R')
                    LeftLeft = true;
            }
            else {
            ...
            if(!node->parent->right || node->parent->right->color == 'B') {
                if(node->left && node->left->color == 'R')
                    RightRight = true;
                else if(node->right && node->right->color == 'R')
                    LeftRight = true;
